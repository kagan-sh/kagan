...
async function spawnHelper(
  ...
  try {
    ...
    const eligible = role === "intake" || (!view.role && !session?.parentID && Boolean(view.worktree))
    if (view.boardTask !== true || !eligible || before.outcome !== undefined || before.sessionID !== undefined) return
...
export async function onEnterBacklog(
  ...
  await spawnHelper(input, "intake", sessionID, options, async (session) => {
...
async function collectCheck(
  ...
  const diffs = await worktreeDiffs(bunGitRunner, worktree, baseBranch ?? "HEAD")
...
export async function onEnterReview(
  ...
  await spawnHelper(input, "validator", sessionID, options, async (session) => {
    const view = kagan(session?.metadata)
