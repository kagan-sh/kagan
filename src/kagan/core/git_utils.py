...
from kagan.core.adapters.process import ProcessResult, ProcessRetryPolicy, run_exec_capture
...
async def _run_git(
    *args: str,
    ...
    increment_counter("core.git.command.calls", fields={"command": command})
    with timed_operation("core.git.command.duration_ms", fields={"command": command}):
        result = await run_exec_capture(
            "git",
            *args,
            cwd=repo_root,
            timeout=timeout,
            retry_policy=_GIT_RETRY_POLICY,
        )
    if result.returncode != 0:
        increment_counter(
            "core.git.command.nonzero_returncode",
            fields={"command": command, "returncode": result.returncode},
        )
    return result
...
async def get_git_user_identity() -> tuple[str, str]:
    ...
    try:
        if not name:
            name_result = await _run_git("config", "--get", "user.name")
            if name_result.returncode == 0:
                name = name_result.stdout_text().strip()

        if not email:
            email_result = await _run_git("config", "--get", "user.email")
            if email_result.returncode == 0:
                email = email_result.stdout_text().strip()
    except FileNotFoundError:
        pass
...
async def check_git_user_configured() -> tuple[bool, str | None]:
    ...
    try:
        ...
        name_set = bool(env_name) or (
            name_result.returncode == 0 and name_result.stdout_text().strip()
        )
        email_set = bool(env_email) or (
            email_result.returncode == 0 and email_result.stdout_text().strip()
        )
...
async def has_git_repo(repo_root: Path) -> bool:
    ...
    try:
        ...
        return result.returncode == 0 and result.stdout_text().strip() == "true"
...
async def has_commits(repo_root: Path) -> bool:
    ...
    try:
        result = await _run_git("rev-parse", "HEAD", repo_root=repo_root)
...
def _ensure_gitignored(repo_root: Path) -> tuple[bool, bool]:
    ...
    - .gemini/settings.json (Gemini project MCP config)
...
async def init_git_repo(repo_root: Path, base_branch: str) -> GitInitResult:
    ...
    Kagan patterns include: .mcp.json, opencode.json, .gemini/settings.json,
    kagan*.mcp.json, *kagan.json
