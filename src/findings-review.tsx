...
export const RECALL_PROMPT = "In one line — what does this change do, and what breaks if this finding is right?"
...
export function detailHeader(finding: Finding, index: number, total: number): string {
  return `finding ${index + 1}/${total} · ${finding.category ?? "finding"} · ${finding.severity ?? "unscored"} · confidence ${finding.confidence ?? "?"}/10`
}

export function locationMarker(finding?: Finding): string | undefined {
