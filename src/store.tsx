...
export function rootSessions(sessions: readonly BoardSession[]): BoardSession[] {
  return sessions.filter(
    (session) => !session.parentID && kagan(session.metadata).boardTask === true && session.time.archived === undefined,
  )
}

export function attachChildren(roots: readonly BoardSession[], sessions: readonly BoardSession[]): BoardCard[] {
...
export function sortSessionsByOrder(sessions: readonly BoardSession[], order: readonly string[]): BoardSession[] {
...
export function sortNeedsHumanFirst(sessions: readonly BoardSession[]): BoardSession[] {
...
export type BoardNotice = TuiToast & { key: string }

export function noticeDuration(toast: TuiToast): number {
  return toast.duration ?? (toast.variant === "error" ? 10000 : 5000)
}

export type HelperFailureNotice = {
  sessionID: string
  taskNumber?: number
...
export type AwaitingInputNotice = {
  sessionID: string
  taskNumber?: number
  permissionID: string
  title: string
