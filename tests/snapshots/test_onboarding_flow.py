...
These tests cover:
- OnboardingScreen initial appearance
- Config file creation on submit (focused state)
- WelcomeScreen with CWD suggestion banner
- WelcomeScreen without CWD banner
...
class TestOnboardingFlow:
    ...
    def test_onboarding_01_initial_screen(
        self,
        snap_compare: Any,
        ...
        class OnboardingTestApp(App):
            """Test app that directly mounts OnboardingScreen."""
    ...
    @pytest.mark.snapshot
    def test_onboarding_05_submit_creates_config(
        self,
        snap_compare: Any,
        snapshot_terminal_size: tuple[int, int],
    ) -> None:
        """Submit button click shows button focused state before submission.

        Note: The actual config file creation is tested in the functional tests.
        This snapshot test verifies the UI state when the submit button is
        focused and ready to be clicked.
        """
        from textual.app import App, ComposeResult
        ...
        async def run_before(pilot: Pilot) -> None:
            await pilot.pause()
            # Tab to continue button (skip agent, branch, auto mode switch)
            ...
            await pilot.press("tab")
            await pilot.pause()
            # Button is now focused, ready to submit
    ...
    @pytest.mark.snapshot
    def test_welcome_06_with_cwd_suggestion(
        self,
    ...
    @pytest.mark.snapshot
    def test_welcome_07_without_cwd_suggestion(
        self,
        snapshot_project: SimpleNamespace,
        mock_acp_agent_factory: MockAgentFactory,
        snap_compare: Any,
