...
7. IF a registry request, manifest validation, download, import, or cache-path validation fails THEN
   ...
   footer.
8. WHEN promotion of the prepared wrapper fails in-process after the current wrapper was moved to backup THEN Kagan SHALL restore the current wrapper immediately; WHEN promotion was interrupted by process exit THEN Kagan SHALL restore the current wrapper from backup on the next launch during cleanup.
