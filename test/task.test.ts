...
describe("kagan() metadata view", () => {
  test("rawKagan stays lossless — unknown keys survive for the merge path", () => {
    const raw = rawKagan({ kagan: { worktree: "/x", someUnknownKey: 42 } }) as Record<string, unknown>
    expect(raw).toEqual({ worktree: "/x", someUnknownKey: 42 })
  })
...
describe("canRetryHelper", () => {
  test("false once the helper has succeeded", () => {
    expect(canRetryHelper({ kagan: { intakeOutcome: "ran" } }, "intake")).toBe(false)
  })

  test("true when the outcome is failed", () => {
    expect(canRetryHelper({ kagan: { validatorOutcome: "failed" } }, "validator")).toBe(true)
  })

  test("true when a helperError is recorded for the role, even before the outcome flips", () => {
    expect(canRetryHelper({ kagan: { helperError: { role: "validator", message: "boom" } } }, "validator")).toBe(true)
  })

  test("true when spawned and stuck without an outcome (sessionID set, outcome undefined)", () => {
    expect(canRetryHelper({ kagan: { validatorSessionID: "v1" } }, "validator")).toBe(true)
  })

  test("false when nothing has started", () => {
    expect(canRetryHelper({ kagan: {} }, "intake")).toBe(false)
    expect(canRetryHelper(undefined, "validator")).toBe(false)
  })
