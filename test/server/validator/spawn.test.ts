...
import { spawnValidator } from "../../../src/server/validator/spawn"
import type { Intake } from "../../../src/domain/task/intake"
import type { CheckResult } from "../../../src/checks/runner"
...
describe("spawnValidator", () => {
  ...
  test("rotates to the second different-provider model by generation", async () => {
    let options: unknown
    await spawnValidator(
      mockInput((o) => (options = o)),
      "parent-1",
      ...
      },
      {
        validatorModels: [
          { providerID: "openai", modelID: "gpt-5" },
          { providerID: "anthropic", modelID: "claude-4" },
          { providerID: "google", modelID: "gemini-2" },
        ],
      },
    )
    expect((options as { body?: Record<string, unknown> }).body?.model).toEqual({
  ...
  test("falls back to the full pool by generation when every model shares the builder's provider", async () => {
    let options: unknown
    await spawnValidator(
      mockInput((o) => (options = o)),
      "parent-1",
