...
import { CHECK_COMMAND_TIMEOUT_MS, runCommandPlan, truncateCheckResultForMetadata } from "../../src/checks/runner"
...
describe("runCheckCommand", () => {
  async function runCheck(command: string, cwd: string) {
    const result = await runCommandPlan([{ name: "check", cwd: ".", command }], cwd, () => true)
    if (!result) throw new Error("expected a check result")
    return result
  }

  test("captures exit code and combined stdout + stderr", async () => {
    const result = await runCheck('echo "hello"; echo "err" >&2', "/tmp")
  ...
  test("keeps only the last 4000 characters of output", async () => {
    const result = await runCheck("yes '*' | head -n 5000 | tr -d '\\n'", "/tmp")
    expect(result.steps?.[0]?.output).toHaveLength(4000)
  })

  test("records a non-zero exit code honestly", async () => {
    const result = await runCheck('echo "warn" >&2; exit 7', "/tmp")
    expect(result.exitCode).toBe(7)
    expect(result.output).toContain("warn")
  })

  test("returns exitCode null on timeout without throwing", async () => {
    const result = await withCheckTimeout(1, () => runCheck("sleep 5", "/tmp"))
    expect(result.exitCode).toBeNull()
    expect(result.output).toContain("timed out")
  })

  test("keeps captured output on timeout", async () => {
    const result = await withCheckTimeout(50, () => runCheck("echo partial; exec sleep 5", "/tmp"))
    expect(result.exitCode).toBeNull()
    expect(result.output).toContain("timed out")
  ...
  test("resolves by the deadline even when a background child outlives the killed shell", async () => {
    ...
    const result = await withCheckTimeout(500, () => runCheck("sh -c 'sleep 30 & sleep 30'", "/tmp"))
  ...
  test("returns exitCode null on an unspawnable command without throwing", async () => {
    const result = await runCheck("echo ok", "/definitely-not-a-real-directory")
    expect(result.exitCode).toBeNull()
