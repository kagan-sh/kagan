...
describe("slug helpers", () => {
  test("slugifyTitle normalizes and truncates", () => {
    expect(slugifyTitle("  Add Rate-Limiting to the API!!  ")).toBe("add-rate-limiting-to-the-api")
    expect(slugifyTitle("***")).toBe("task")
  })

  test("uniqueTaskSlug appends a random suffix", () => {
    const slug = uniqueTaskSlug("Fix bug")
    expect(slug).toMatch(/^fix-bug-[a-z0-9]{4}$/)
  })

  test("taskBranch prefixes with kagan/", () => {
    expect(taskBranch("fix-bug-ab12")).toBe("kagan/fix-bug-ab12")
  })
})
...
describe("parseNumstat", () => {
  test("parses additions/deletions and treats binary markers as 0", () => {
    const map = parseNumstat("2\t3\tsrc/a.ts\n-\t-\tassets/logo.png\n")
    expect(map.get("src/a.ts")).toEqual({ additions: 2, deletions: 3 })
...
describe("parseNameStatus", () => {
  test("maps status codes and defaults unknowns to modified", () => {
    const map = parseNameStatus("A\tsrc/new.ts\nD\tsrc/gone.ts\nM\tsrc/edit.ts\nX\tsrc/weird.ts\n")
    expect(map.get("src/new.ts")).toBe("added")
    expect(map.get("src/gone.ts")).toBe("deleted")
    expect(map.get("src/edit.ts")).toBe("modified")
    expect(map.get("src/weird.ts")).toBe("modified")
  })
})

describe("splitPatchByFile", () => {
  test("splits a combined patch into per-file sections keyed by the b/ path", () => {
    const patch =
      "diff --git a/src/z.ts b/src/z.ts\n@@ -1 +1 @@\n-old\n+new\n" +
      "diff --git a/src/a.ts b/src/a.ts\n@@ -0,0 +1 @@\n+added\n"
    const map = splitPatchByFile(patch)
    expect([...map.keys()]).toEqual(["src/z.ts", "src/a.ts"])
    expect(map.get("src/z.ts")).toContain("+new")
  })
})
